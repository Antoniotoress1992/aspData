...
function ig_CreateWebMonthView(props)
{
    ...
    if(!ig_DayOrientedView.prototype.isPrototypeOf(ig_WebMonthView.prototype))
    {
		...
		ig_WebMonthView.prototype._onClick = function(src, evt) 
		{
			...
			if(next || attr == "PREV")
			{
				if(!this.fireEvent(next ? "navigatenext" : "navigateprevious", evt, next ? 1: -1))	
				{
					...
					var daysToMove = next ? 28 : -28;
					var currentMonth = new Date(info.getActiveDay().getTime() + 3600000 * 24 * daysToMove);
		...
		ig_WebMonthView.prototype._onMousedown = function(src, evt) 
		{
			...
			{	
				...
				if(srcheader == this._selectedHeader.elem)
				{
					...
					if(this._selectedAppt)
					{
						
						var appResKey = scheduleInfo.getActivities().getItemFromKey(this._selectedAppt.key);
						if (appResKey)
							appResKey = appResKey.getResourceKey();
							if (appResKey && appResKey.length > 0 && appResKey != scheduleInfo.getActiveResourceDataKey())
							{
		...
		ig_WebMonthView.prototype._onTouchstart = function (src, evt) 
		{
			//suppresses mousedown, mouseup, click events
			event.preventDefault();

			//performs selection
			this._onMousedown(src, evt);

            for (var id in this._tapAndHoldTimers)
            {   
                clearTimeout(this._tapAndHoldTimers[id]);
            }

			//open Appointment dialog after one second tapping and holding
			var self = this;
            var timerId = setTimeout(function () {
				var newEvnt = new Object();
				newEvnt.type = "dblclick";
				self._onDblclick.call(self, src, newEvnt);
            }, 1000);
            this._tapAndHoldTimers[timerId] = timerId;
		}

		ig_WebMonthView.prototype._onTouchend = function (src, evt) 
		{
			event.preventDefault();
			...
			//invokes click so pressiong on prev/next month arrows still work
			var newEvnt = new Object();
			newEvnt.type = "click";
			this._onClick(src, newEvnt);
		}
