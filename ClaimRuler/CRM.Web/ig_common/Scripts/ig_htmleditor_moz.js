...
function iged_init(ids,p1,p2,p3,p4,p5)
{
	...
	{
		try{return this._sel().getRangeAt(0).toString();}catch(ex){}
		return '';
	}
	...
	o.getCaret=function()
	{
		try
		{
			return o._sel().getRangeAt(0).endOffset;
		}catch(ex){}
		return -1;
	}
...
//	}
	...
	o._htSet=function()
	{
		var e0=o._elem0,h=e0?e0.style.height:null,td=iged_el(id+"_td_"),val=iged_el(id+"_ts");
		if(td&&e0&&tb&&h&&h.indexOf('px')>0)try
		{
			...
			h=parseFloat(h);
			if(val+td.offsetHeight>h)td.height=(h-val)+"px";
		}catch(ex){}
...
function iged_insNodeAtSel(insertNode,o,fr)
{
	...
	o=o||iged_all._cur;
	var sel=o._sel(),range=sel.getRangeAt(0);
