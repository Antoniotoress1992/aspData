...
function IGBrowser() 
{
    ...
    this.IsIE9Plus          = this.AgentName.indexOf("msie") != -1 && navigator.appVersion && navigator.appVersion.indexOf("MSIE") != -1 && parseInt(navigator.appVersion.split("MSIE")[1]) >= 9;
	...
	{ 
		if ((this.IsFirefoxOrSafari || IGB.IsIE9Plus) && document.documentElement) h += "px";
		if (this.IsNetscape) 
		{
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIE9Plus) && document.documentElement) w += "px";
		if (this.IsNetscape) 
		{
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIE9Plus) && document.documentElement) x += "px";
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIE9Plus) && document.documentElement) y += "px";
...
function IGWindowViewer(id, imageId, vuid, srcBounds, destBounds) 
{
	...
	this.Render=function()
	{
		...
		if ((IGB.IsFirefoxOrSafari || IGB.IsIE9Plus) && document.documentElement)
		{
			w += "px";
			h += "px";
		}
...
function IGUltraChart(id, imageUrl, uniqueId)
{
	...
	this.CreateComponents = function (vals) 
    {
        ...
        // hide the crosshair if the chart is in UpdatePanel and the crosshair is not cleared.
        var crosshair_horizontal_ref_body = IGB.GetObject(this.ID + "_HairHorizontalBody");
        if (crosshair_horizontal_ref_body) {
            crosshair_horizontal_ref_body.style.visibility = 'hidden';
            crosshair_horizontal_ref_body.parentNode.removeChild(crosshair_horizontal_ref_body);
        }
        var crosshair_vertical_ref_body = IGB.GetObject(this.ID + "_HairVerticalBody");
        if (crosshair_vertical_ref_body) {
            crosshair_vertical_ref_body.style.visibility = 'hidden';
            crosshair_vertical_ref_body.parentNode.removeChild(crosshair_vertical_ref_body);
        }
	...
	this.onallevent=function(evt, id, args)
	{
		...
		if (evt.type == "mousemove" && (IGB.IsNetscape6 || IGB.IsMac || IGB.IsOpera || IGB.IsSafari || IGB.IsIE9Plus))
		{
