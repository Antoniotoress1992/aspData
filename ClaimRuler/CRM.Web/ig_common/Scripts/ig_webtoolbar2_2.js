...
function igtbar_initialize(tbName,id,propArray,itemsArray,eventsArray)
{
	this.loading = true;
	...
	this.clientId = id;
	id = "igtbar_" + tbName;
	this.ClientSideEvents = new igtbar_initEvents(eventsArray);
	this.Id=tbName;
		...
		try
		{
			if(this.CausesValidation&&typeof(Page_Validators)=="object"){
				...
				for(var q=0;q<pgV.length;++q){
					...
					var ok=pgV[q].isvalid;
					if(ok!=null&&ok==false)bail=true;
				}
				if(bail)
				{
...
function igtbar_mouseUp(src,evnt,enabled)
{
	var tbName=src.id.split("_");
	var tb=igtbar_getToolbarById(tbName[0]);
	var b=igtbar_getItemById(src.id),down=tb?tb._downB:null;
	if(down)delete tb._downB;
	if(!tb||tb.loading||!enabled||!tb.Enabled||!b||!b.Enabled||(down!=b && evnt.type!='keyup'))return;
	var eventObj=new ig_EventObject();
	eventObj.event=evnt;
