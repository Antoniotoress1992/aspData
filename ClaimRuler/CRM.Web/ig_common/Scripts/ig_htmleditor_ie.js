...
function iged_init(ids,p1,p2,p3,p4,p5)
{
	...
	o._onApplyStyle=function(e)
	{
		...
		if(!cr||!cr.select||!str||str.length<2)return;
		var txt=cr.text||iged_stripTags(cr.htmlText||"");
		if(!txt)return;
		str=(str.indexOf(":")>1?"style='":"class='")+str;
		str="<font "+str+"'>"+txt.replace(/\n/g, '').replace(/\r/g, '')+"</font>";
		cr.select();
