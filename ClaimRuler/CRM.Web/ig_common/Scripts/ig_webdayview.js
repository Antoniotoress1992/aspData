...
function ig_CreateWebDayView(props)
{
	if(!ig_WebControl.prototype.isPrototypeOf(ig_WebDayView.prototype))
	{
		...
		ig_WebDayView.prototype._initProp = function(props)
		{
			...
			ig_shared.addEventListener(elem, "touchstart", ig_handleEvent);
		...
		ig_WebDayView.prototype._fixLbls = function()
		{
			...
			//vvvvvvvvvvvvvvvvvv
			// verify that height of header-td is not larger than heights of it children tables (top1/2)
			var e0 = this._bottom, height = e0 ? e0.style.height : null;
			height = (height && height.indexOf("px") > 0) ? parseInt(height) : 0;
			var height0 = height ? (this._top1 ? this._top1.offsetHeight : 0) + (this._top2 ? this._top2.offsetHeight : 0) : 0;
			if (height0 > 0)
			{
				height0 = this._element.clientHeight - height0 + 1;
				if (height0 - height > 0 && height0 - height < 20)
					e0.style.height = height0 + "px";
			}
			//^^^^^^^^^^^^^^^^^^
			// :old logic with validation of time-labels:
			var tbl = this._tblRows, tblH = tbl.offsetHeight, rows = tbl.rows, len = rows.length;
			// first time-row-label
			e0 = rows[0].cells[0];
			height0 = e0 ? e0.offsetHeight : 0;
			// 0.9/1-reduce for bottom border of row-labels
			height = Math.floor(tblH / len - 0.9);
			// last time-row-label
			var e1 = rows[len - 1].cells[0];
			// chrome:
			if(height0 && e1 && height0 + 1 < e1.offsetHeight) for(var i = 0; i < len; i++)
			{
				...
				if(e1)
				{
					...
					{
						if (height0 > height + 5)
							height0 -= e0.clientHeight;
						else
							height0 -= height;
						if (height < height0 + 5 || height0 > 5)
							height0 = 1;
						height -= height0;
						i--;
					}
			...
			tblH = tbl.offsetHeight;
			e0 = this._days;
			i = e0.length;
			// set height of TABLE-container of rows to the height of parent TD
			while (i-- > 0)
			{
				e1 = e0[i].tblRows;
				if (e1)
					e1.style.height = tblH + 'px';
			}
		}
		...
		ig_WebDayView.prototype._onResize = function(type)
		{
			...
			var fs0 = this._fixScroll0, fs1 = this._fixScroll1 || fs0;
			if(fs0 && bot)
			{
				var display = (bot.offsetHeight >= tbl.offsetHeight) ? "none" : "";
				if(fs0.style.display != display)
				{
					fs0.style.display = fs1.style.display = display;
					
					if (ig_shared.IsIE && display && fs0.offsetWidth > 10)
						fs0.style.display = fs1.style.display = "";
				}
			}
		...
		ig_WebDayView.prototype._onHandleEvent = function(src, evt) {
		    ...
		    if (!src || !this._info)
		    	return;
		    if (type == "touchstart") {
		    	var oldTouchTime = this._oldTouchTime;
		    	var touchTime = this._oldTouchTime = new Date().getTime();
		    	var oldSrc = this._oldTouchSrc;
		    	var touchSrc = this._oldTouchSrc = src;
		    	if (!oldTouchTime || touchTime - oldTouchTime > 500 || touchSrc !== oldSrc) {
		    		return;
		    	}
		    	ig_cancelEvent(evt);
		    	var newEvnt = new Object();
		    	newEvnt.type = "dblclick";
		    	newEvnt.clientY = y;
		    	this._onHandleEvent(this._oldTouchSrc, newEvnt);
		    }
